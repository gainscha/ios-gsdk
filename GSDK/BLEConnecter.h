...
/**
 *  @discussion 指定在CBCharacteristic上执行哪种写入类型，默认使用CBCharacteristicWriteWithResponse
...
-(void)connectPeripheralFormUUID:(NSString *_Nullable)identifier timeout:(NSUInteger)time connectState:(void(^_Nullable)(ConnectState state))connectState;
 ...
 */
-(void)writeValue:(NSData *_Nonnull)data forCharacteristic:(nonnull CBCharacteristic *)characteristic type:(CBCharacteristicWriteType)type;
@end
